<template>
    <nav class="flex flex-wrap items-center justify-between bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 px-4 py-3 shadow-lg">
        <!-- Branding -->
        ...
        <button @click="showMobileMenu = !showMobileMenu" class="flex items-center px-2 py-1 text-white focus:outline-none md:hidden">
        ...
        <div class="hidden items-center gap-6 md:flex">
            ...
            <Link :href="route('projects.list')" class="font-medium text-white transition hover:text-gray-900">Projects</Link>
        ...
        <!-- Mobile Nav -->
        <div v-if="showMobileMenu" class="mt-3 w-full md:hidden">
            <div class="flex flex-col gap-2 rounded-lg bg-yellow-500 p-3">
                <Link :href="route('home')" class="font-medium text-white transition hover:text-gray-900">Home</Link>
                <div v-if="user" class="mt-2">
                    <button
                        @click="showDropdown = !showDropdown"
                        class="flex w-full items-center gap-2 rounded-lg bg-yellow-700 px-3 py-2 font-semibold text-white transition hover:bg-yellow-800 focus:outline-none"
                    >
                        <span class="font-semibold">{{ user.name || 'Account' }}</span>
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
    ...
    <footer
        class="mt-8 flex flex-col items-center justify-between gap-2 bg-yellow-700 px-4 py-4 text-center text-white md:flex-row md:gap-0 md:px-6 md:text-left"
    >
